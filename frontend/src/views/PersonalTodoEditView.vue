    ...
    <main class="page-content">
      ...
      <template v-else-if="form">
        <div class="form-header">
          ...
          <h1>Edit Todo</h1>
          <p>Update the todo details below.</p>
        </div>
        ...
        <form class="task-form" @submit.prevent="handleSubmit">
          ...
          <div class="form-actions">
            ...
            <button type="submit" class="btn-primary" :disabled="todoStore.saving">
              ...
              {{ todoStore.saving ? 'Saving...' : 'Save Changes' }}
            </button>
          </div>
        </form>
      </template>
      ...
      <div v-else class="empty-state">
        ...
        <p>The todo you're looking for doesn't exist or has been deleted.</p>
        <router-link to="/personal-todos" class="btn-primary">Back to Personal Todos</router-link>
      </div>
    </main>
  </div>
</template>
...
export default {
  ...
  setup() {
    ...
    async function handleSubmit() {
      ...
      const updated = await todoStore.updateTodo(Number(route.params.id), payload)
      if (updated) {
        router.push('/personal-todos')
      }
    }

    return { todoStore, loading, form, handleSubmit }
  },
}
</script>
